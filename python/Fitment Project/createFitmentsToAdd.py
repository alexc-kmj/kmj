...
def make_csv_file(CSV_NAME,DEL):
    return csv.writer(open(CSV_NAME, "w"),delimiter=DEL)
...
def make_SQL_call(ODBC_CONN,SQL,SQL_VAR):
    ...
    #print('Make: ' + str(len(make)))
    #print('Engine: ' + str(len(engine)))
    
    while len(make) > len(engine):
        engine.append('')
...
CSV_TO_WRITE = 'Sixbit_' + time.strftime('%m%d') + '-Fitments_To_Add_OldMethod.csv'        # Name of the CSV we will output the part numbers too
...
SQL_Fitment = "SELECT N.C.value('Name[1]', 'nvarchar(max)') AS Field, N.C.value('Value[1]', 'nvarchar(max)') AS Value FROM SixBit_KMJPERFORMANCE.dbo.CompatibilitySets \
    CROSS APPLY CompatibilitySetDefinition.nodes('/Compatibilities/Compatibility/NameValue') N(C) INNER JOIN SixBit_KMJPERFORMANCE.dbo.ItemsEbay ON CompatibilitySets.CompatibilitySetID = ItemsEbay.CompatibilitySetID \
    INNER JOIN SixBit_KMJPERFORMANCE.dbo.Inventory ON Inventory.ItemID = ItemsEbay.ItemID Inner Join SixBit_KMJPERFORMANCE.dbo.Listings On Listings.ItemID = Inventory.ItemID Inner Join SixBit_KMJPERFORMANCE.dbo.ListingsEbay On Listings.ListingID = ListingsEbay.ListingIDLink \
    WHERE Inventory.SKU = ? AND ItemsEbay.CompatibilitySetID <> 1 AND Listings.StatusID = 2000";
...
print("Creating New Fitments_To_Add_OldMethod.csv")

#write headers
#toWrite = []
#toWrite.append('fitid')
#toWrite.append('sku')
#toWrite.append('year')
#toWrite.append('make')
