...
CSV_TO_READ = 'updateSixbitQuantitiesByListingID_ARC.csv'  # This assumes your part numbers are in the first column
...
for row in csv_Sixbit:
    ...
    if ( int(Quantity) > int(QtyUncommited) ):
        ...
        run_sql_update_statement(ODBC_CONN,"INSERT INTO SixBit_HCR.dbo.Purchases (InventoryID, Quantity, PurchaseSource) VALUES (?, ?, 27)", InventoryID, Quantity)
    ...
    run_sql_statement(ODBC_CONN,"UPDATE SixBit_HCR.dbo.Listings SET RevisionPending = 1, QtyToList = NumberSold + ?, OutOfStock = ? WHERE ListingID = ?", Quantity, OutOfStock, ListingID)
