...
CSV_TO_READ = 'MapPrice.csv'  # This assumes your part numbers are in the first column
...
for row in csv_Amazon:
    ...
    #print ListPrice
    MapPrice_float = float(MapPrice)
    print (PLC + ' ' + PartNumber + ' --> ' +  str(MapPrice_float))
    ...
    run_sql_statement(ODBC_CONN,"UPDATE ImasterECommerce SET MapPrice = ? FROM ImasterECommerce INNER JOIN Imaster ON Imaster.PartID = ImasterECommerce.PartID INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?	",MapPrice_float,PLC, PartNumber)
