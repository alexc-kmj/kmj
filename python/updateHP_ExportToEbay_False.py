...
CSV_TO_READ = 'ExportToEbay.csv'  # This assumes your part numbers are in the first column
...
for row in csv_Amazon:
    ...
    #print ListPrice
    print (PLC + ' ' + PartNumber + ' --> ExportToEbay = False')
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET ExportToEbay = 0 FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
