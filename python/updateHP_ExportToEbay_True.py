...
for row in csv_Amazon:
    ...
    #print ListPrice
    print (PLC + ' ' + PartNumber + ' --> ExportToEbay = True')
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET ExportToEbay = 1 FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
