...
for row in csv_Amazon:
    ...
    print (PLC + ' ' + PartNumber + ' --> PopCode Normal')
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET PopCode = 'NORMAL' FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
