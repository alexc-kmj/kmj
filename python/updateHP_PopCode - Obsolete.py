...
for row in csv_Amazon:
    ...
    print (PLC + ' ' + PartNumber + ' --> PopCode Obsolete')
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET PopCode = 'OBSOLETE' FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
