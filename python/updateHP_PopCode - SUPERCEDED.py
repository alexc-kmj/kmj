...
CSV_TO_READ = 'PopCode.csv'      # This assumes your part numbers are in the first column
...
for row in csv_Amazon:
    ...
    print (PLC + ' ' + PartNumber + ' --> PopCode SUPERCEDED')
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET PopCode = 'SUPERCEDED' FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
