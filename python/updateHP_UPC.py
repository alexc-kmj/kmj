...
CSV_TO_READ = 'UPC.csv'  # This assumes your part numbers are in the first column
...
for row in csv_Amazon:
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET UPCCode = ? FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", UPC,PLC, PartNumber)
