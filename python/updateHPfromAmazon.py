...
#        Split PLC & PartNumber, Version True           #
#########################################################
...
for row in csv_Amazon:
    ...
    print (PLC + ' ' + PartNumber + ' --> Amazon True')
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET ExportToAmazon = 1 FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
