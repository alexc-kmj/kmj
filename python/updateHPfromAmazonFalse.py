...
CSV_TO_READ = 'isOnAmazon.csv'      # This assumes your part numbers are in the first column
                                    # Also your parts have the PLC and Part Number
                                    # ie. LND EX-0122-07.
CSV_TO_READ_DEL = '\t'              # The delimiter on the CSV you are reading
ODBC_CONN_NAME = 'HPCommerce'       # ODBC connection name
ODBC_USER = ''                      # ODBC user name, '' if there is no user required
...
for row in csv_Amazon:
    ...
    print (PLC + ' ' + PartNumber + ' --> Amazon False')
    
    #make call to database
    #run_sql_statement(ODBC_CONN, "UPDATE Imaster SET Is_On_Amazon = FALSE FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE Plc.PLC = ? AND Imaster.PartNumber = ?", [PLC_CODE], [PART_NUMBER])
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET ExportToAmazon = 0 FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
