...
CSV_TO_READ = 'exportToWeb.csv'  # This assumes your part numbers are in the first column
...
for row in csv_Amazon:
    ...
    #print ListPrice
    print (PLC + ' ' + PartNumber + ' --> ExportToWeb ' +  ExportToWeb)
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET ExportToWeb = ? FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?",ExportToWeb,PLC, PartNumber)
