...
for row in csv_Amazon:
    ...
    print (PLC + ' ' + PartNumber + ' --> Magento True')
    
    #make call to database
    #run_sql_statement(ODBC_CONN, "UPDATE Imaster SET Is_On_Magento = TRUE FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE Plc.PLC = ? AND Imaster.PartNumber = ?", [PLC_CODE], [PART_NUMBER])
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET Is_On_Magento = TRUE FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?", PLC, PartNumber)
